...
namespace ImGui
{
    ...
    IMGUI_API bool          InputTextString(const char* label, std::string* str, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_NoPopupHierarchy              = 1 << 3,   // IsWindowFocused(): Do not consider popup hierarchy (do not treat popup emitter as parent of popup) (when used with _ChildWindows or _RootWindow)
    ImGuiFocusedFlags_DockHierarchy                 = 1 << 4,   // IsWindowFocused(): Consider docking hierarchy (treat dockspace host as parent of docked window) (when used with _ChildWindows or _RootWindow)
